...
import { getBazaarPage, loadDemoBazaar, refreshBazaar } from '@/lib/api';
...
export function BazaarScreener({ initialParams = {} }: { initialParams?: Record<string, string | string[] | undefined> }) {
  ...
  const loadDemo = async () => {
    ...
    setDemoLoading(true);
    try {
      await loadDemoBazaar();
      await query.refetch();
    } catch (error) {
      setDemoError(error instanceof Error ? error.message : 'The local demo dataset could not be loaded.');
    } finally {
  ...
  return (
    <>
      ...
      {query.data?.freshness.source === 'demo' && <div className="alert-banner alert-demo"><span>DEMO DATA: these values are deterministic local examples, not live market prices.</span></div>}
      ...
      {demoError && <div className="alert-banner alert-error"><span>{demoError}</span></div>}
