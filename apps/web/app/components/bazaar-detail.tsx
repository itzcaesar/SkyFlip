...
import { MarketHistoryChart } from '@/app/components/market-history-chart';
...
export function BazaarDetail({ productId }: { productId: string }) {
  ...
  const [historyRange, setHistoryRange] = useState<'24h' | '7d' | '30d' | '90d'>('7d');
  const detail = useQuery({ queryKey: ['bazaar', 'detail', productId], queryFn: () => getBazaarDetail(productId), refetchInterval: 30_000 });
  const history = useQuery({ queryKey: ['bazaar', 'history', productId, historyRange], queryFn: () => getBazaarHistory(productId, 'buy_order_to_sell_order', historyRange), refetchInterval: 60_000, enabled: detail.isSuccess });
